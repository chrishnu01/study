module.exports = function generateQuotationTemplate(data) {
  const {
    clientName,
    siteName,
    date,
...
.account-details {
  ...
  <body>
    <div class="container">
      ...
      <p>Dear Sir,</p>
      <p>Please find below our best rates for rental purpose:</p>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Qty</th>
            <th>Unit</th>
            <th>Rent Rate in Rs.</th>
            <th>UM</th>
            <th>${daysRsInAdvanceDays} Days Rent Rs. in Advance</th>
          </tr>
        </thead>
        <tbody>
          ${items.map((item, i) => `
            <tr>
              ...
              <td>${item.unit || ""}</td>
              <td>${item.rentRatePerPiecePerDay || 0}</td>
              <td>${item.uM || ""}</td>
              <td class="text-right">${item.rsInAdvance || 0}</td>
            </tr>`).join('')}
        </tbody>

         <tfoot>
    <tr>
      <td colspan="7" style="text-align:left; font-weight:bold;">Total Before GST:</td>
      <td class="text-right"><strong>₹${totalBeforeGST}</strong></td>
    </tr>

    <tr>
      <td colspan="7" style="text-align:left; font-weight:bold;">GST (${gstPercentage}%):</td>
      <td class="text-right"><strong>₹${totalGST}</strong></td>
    </tr>

    <tr>
      <td colspan="7" style="text-align:left; font-weight:bold; background:#f8f8f8;">Total After GST:</td>
